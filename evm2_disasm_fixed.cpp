...
pair<Op, unsigned> readOpcode(BitReader& br) {
    string bits;
...
optional<Arg> readDataArg(BitReader& br) {
...
vector<Instruction> disassembleCode(const vector<uint8_t>& code) {
    BitReader br(code);
...
int main(int argc, char** argv) {
    ...
    auto program = disassembleCode(code);

    cout << "Disassembled " << program.size() << " instructions.\n";
    for (size_t i = 0; i < program.size(); ++i) {
        const auto& ins = program[i];
        cout << i << ": bitOffset=" << ins.bitOffset << "  " << opToString(ins.opcode);
        if (!ins.args.empty()) {
            cout << "   ";
            for (size_t j = 0; j < ins.args.size(); ++j) {
                if (j) cout << ", ";
                cout << ins.args[j].toString();
            }
        }
        cout << "\n";
    }
